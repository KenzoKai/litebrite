...
try{
  ...
  await host.getByRole('button',{name:/Blackout/}).click();await guest.waitForTimeout(350);
  await host.getByRole('button',{name:'Zoom in',exact:true}).click();
  const z=await center(host,30,15,1.5);await host.mouse.click(z.x,z.y);await guest.waitForTimeout(350);
  await expect.poll(() => litCells(guest), {timeout:2000,intervals:[50]}).toEqual([15*56+30]);
  ...
  await draw(host);await host.keyboard.press('Escape');await guest.waitForTimeout(350);
  assert.equal(await pixels(host),0);assert.equal(await pixels(guest),0);
  ...
  await host.getByText('CONNECTED · JUST YOU TWO',{exact:true}).waitFor({timeout:45000});
  await guest.getByText('CONNECTED · JUST YOU TWO',{exact:true}).waitFor({timeout:15000});
  await draw(host);await guest.waitForTimeout(350);assert.ok(await pixels(guest)>0);
